...
<div class="d-flex justify-content-end" style="padding:0 10px 10px">
    <form action="{{ route('meritos.show', $meritos->id) }}" method="GET">
        <button type="button" class="btn btn-success" type="submit">Agregar Merito</button>
    </form>
</div>
            ...
            @foreach ($meritos as $merito)
                <tr>
                    ...
                    <td>{{$merito->obligatorio}}</td>
                        ...
                        <td class="text-center" width="10px">
                            <form action="{{ route('meritos.edit', $meritos->id) }}" method="GET">
                                <button class="btn btn-warning px-3 btn-sm" type="submit"><i class="fas fa-edit"></i></button>
                            </form>
                        </td>
                        <td class="text-center" width="10px">
                            <form action="{{ route('meritos.destroy', $meritos->id) }}" method="POST">
                                {{ csrf_field() }}
